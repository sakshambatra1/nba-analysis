...
def get_career_clutch_stats(player_id):
    ...
    try:
        ...
        # Then try last few seasons for newer players
        ...
        stats = {
            ...
            'total_points': int(clutch_games['PTS'].sum()) if not clutch_games.empty else 0,
...
st.markdown("Compare players' career performance in close games (margin ≤ 5 points)")
...
if st.button("Compare Players", type="primary"):
    with st.spinner("Analyzing career clutch performance..."):
        ...
        st.markdown(f"""
        - {better_clutch} has a higher win percentage in clutch games by {win_diff:.1f}%
...
# Footer
st.markdown("---")
st.caption("Data from NBA.com | Close games defined as final margin ≤ 5 points")